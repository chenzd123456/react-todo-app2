...
root.render (
    <div> {/* 必须是一个根DOM节点 */}
        测试Checkbox: {/* 测试check={true}，测试text="checked" */}
        {/* 这里checked已经写死，所以点击也不能变化是否勾选。想要改变必须改变传递给checked的值*/}
        <CheckBox checked={true}
            onChange={
                (checked) => (alert("checked=" + checked))
            }
            text="checked"/> {/* 测试check={false}，测试text="unchecked" */}
        {/* 这里checked已经写死，所以点击也不能变化是否勾选。想要改变必须改变传递给checked的值*/}
        ...
        <TodoListItem id={1}
            completed={true}
            onChange={
                (id, completed) => (alert("id=" + id + " completed=" + completed))
            }
            text="completed"/>
        ...
        <InputBox onEnterKeyUp={
                (value) => (alert("value=" + value))
            }
            onChange={
                (value) => {}
            }
            value="add todo"></InputBox>
